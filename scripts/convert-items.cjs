...
function determineItemType(typeInfo) {
  ...
  if (overallCategory === 'Use') return 'use';
  return 'etc';
}
...
function determineCategory(typeInfo, itemType) {
  ...
  if (itemType === 'use') {
    ...
    if (category && category.includes('Scroll')) return 'scroll';
    if (subCategory === 'Upgrade' || subCategory.includes('Scroll')) return 'scroll';
    return 'consumable';
  }
